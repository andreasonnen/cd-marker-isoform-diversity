...
# Figure 4 uses six precomputed objects:
# # 1. tx_summary # Transcript-level composition summary by cell population: This object contains 
# the relative contribution of each transcript isoform to the total gene-level 
# signal within each immune cell population. 
# # 2. function_main_bundle 
# Transcript-associated functional pathway enrichments generated by the 
# functional characterization pipeline. This table contains pathway-level 
# associations for each gene, transcript isoform, and cell population, including 
# normalized enrichment scores, adjusted P values, direction of association, 
# and pathway/program annotations. 
# # 3. tm_seg2 
# Parsed protein topology predictions for transcript/protein isoforms. For each  transcript,
#amino-acid intervals are annotated as signal peptide, outside or lumenal region, transmembrane helix
#, or inside/cytosolic region. These  coordinates are used to draw predicted membrane topology in
#the protein architecture panel. 
# # 4. ipr_df_pfam 
# Parsed InterProScan domain annotations, including Pfam hits, generated from  the complete protein 
#FASTA using InterProScan. These annotations are used to # display predicted protein domains for 
#each transcript/protein isoform.
# # 5. ab_tx # PrEST antigen-to-transcript coverage results. This object describes which 
# transcript/protein isoforms are covered by Human Protein Atlas PrEST antigen 
# regions and is used to assess antibody antigen representation across isoforms. 
# # 6. cd_markers_localization 
# Predicted subcellular localizations for CD-marker transcript/protein isoforms, 
# generated with DeepLoc. This object is used to display predicted localization
# classes for the selected transcript isoforms.
